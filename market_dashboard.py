...
with st.expander("📖 查看：操盤判讀邏輯 & 交易心法 (點擊展開)", expanded=False):
    ...
    with c1:
        ...
        4. **Z-Score**：> +1.5 代表過度擁擠(易跌)；< -1.5 代表悲觀(易反彈)。
        """)
    with c2:
        st.markdown("""
        ### 🛡️ 2026 交易心法:
        * **部位控管**：當流動性指標（日圓/利率）亮綠燈時，持倉水位應降至 **3~5 成**。
        * **拒絕 FOMO**：融漲末端常見暴漲，須配合「成交量」判斷是否為機構倒貨。
        * **槓鈴配置**：保留 20% 現金或黃金作為保險，防範流動性斷裂的系統性風險。
        """)

# ==========================================
# 2. 核心運算引擎 (Period="5y" 以修復 Z-Score)
# ==========================================
    ...
    for ticker in ticker_list:
        try:
            if ticker in data.columns:
                ...
                if len(series) >= 504: # 兩年以上數據
                    price = series.iloc[-1]
                    ma20 = series.rolling(20).mean().iloc[-1]
                    bias = (price - ma20) / ma20 * 100
                    trend = "🔴強勢" if bias > 0 else "🟢弱勢"
                    z_score = (price - series.tail(504).mean()) / series.tail(504).std()
                    results.append({
...
with tab_ai:
    ...
    if not df_ai.empty:
        ...
        with c1:
            ...
            st.metric("資金擁擠度 (Z-Score)", round(df_ai['Z-Score'].mean(), 2))
        with c2:
...
with tab_tw:
    ...
    st.dataframe(df_high[df_high['現價']>=800][["資產名稱", "趨勢", "乖離率", "Z-Score", "現價"]].sort_values("現價", ascending=False), hide_index=True, use_container_width=True)
