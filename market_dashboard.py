...
with tab5:
    ...
    # 這裡改回最穩定的邏輯：一次下載，如果不成功就顯示錯誤，不要靜默失敗
    ...
    try:
        ...
        # 處理 Close 欄位 (yfinance 有時會回傳多層索引)
        if 'Close' in raw_data.columns:
            data_closes = raw_data['Close']
        else:
            data_closes = raw_data # 萬一只有一層
        ...
        if benchmark_ticker in data_closes.columns:
            ...
            if not bench_series.empty and len(bench_series) > 60:
                ...
                if semi_results:
                    ...
                    st.warning("計算後無數據，可能上市時間不足 60 天")
            else:
                st.error("基準指數 (SPY) 數據長度不足")
        else:
            st.error(f"無法取得基準指數 ({benchmark_ticker}) 數據")
            
    except Exception as e:
        st.error(f"數據下載失敗，請稍後重試: {e}")
