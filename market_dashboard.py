...
with st.expander("📖 新手指南：數據判讀 & 交易心法 (點擊展開)"):
    ...
    with c2:
        st.markdown("""
        ### 🛡️ 交易心法 (How to Act):
        1. **部位控管 (Position Sizing)**：
           * 看到紅燈(多)不代表梭哈。
           * 若 **Tab 3 風險雷達** (資金成本) 亮綠燈(警戒)，即使個股好，持倉也應降至 **3~5 成**。
        2. **事件風險 (Event Risk)**：
           * 程式抓不到「聯準會主席講話」或「地緣政治」。
           * 操作時請搭配 **財經行事曆**，避開重大數據發布前後的劇烈波動。
        3. **勿過度優化 (No Overfitting)**：
           * 模型參數是基於假設 (例如成長率)。
           * 若三個估值模型算出來都說貴，那就是貴，**不要試圖調整參數去騙自己「它還便宜」**。
        """)
