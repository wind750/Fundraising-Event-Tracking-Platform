...
with st.expander("📖 查看：操盤判讀邏輯 & 交易心法 (點擊展開)", expanded=False):
    ...
    with c1:
        ...
        2. **台股戰略**：費半/中小指標亮紅燈代表內外資同步做多。
        3. **風險雷達**：日圓匯率 > 60MA (季線) 為安全(紅)；跌破則為警戒(綠)。
        4. **Z-Score**：基於兩年(504天)統計，衡量籌碼擁擠度。
        """)
    with c2:
        st.markdown("""
        ### 🛡️ 2026 交易心法:
        * **動態汰弱留強**：千金股門檻定為 800 元，實時監控內資大戶撤退訊號。
        * **避開擁擠陷阱**：Z-Score > +1.5 時，即使價格創新高，也應警惕融漲末端。
        * **真金白銀預測**：預測市場機率達 90% 以上時，代表事件已被華爾街完全定價 (Priced In)。
        """)

# ==========================================
# 2. 數據下載 (補回廣度與信用風險所需資產)
# ==========================================
...
with t_poly:
    ...
    def fetch_top_events():
        try:
            # 透過 AllOrigins 的 proxy 繞過 Polymarket 的 Cloudflare 阻擋
            target_url = "https://gamma-api.polymarket.com/events?limit=5&active=true&closed=false&order=volumeNum"
            proxy_url = f"https://api.allorigins.win/raw?url={urllib.parse.quote(target_url)}"
            
            # 使用 Proxy 取得原始 JSON 數據
